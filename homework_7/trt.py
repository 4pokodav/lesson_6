...
def run_trt_inference(engine, dataloader, input_shape):
    context = engine.create_execution_context()
    context.set_input_shape('input', (batch_size, *input_shape))
    inputs, outputs, stream = allocate_buffers(context)

    times = []
    for images, _ in tqdm(dataloader, desc="Running inference"):
        input_data = images.contiguous().view(-1).cpu().numpy()
    ...
    print(f"\nСреднее время инференса: {avg_time:.2f} ms / batch ({batch_size} samples)")
    return avg_time
...
def test_pure_trt_model(
    ...
    try:
        ...
        results = run_test(
            ...
            dataset=dataloader.dataset if dataloader else None,
            timer_type='cuda'
        )
    finally:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, default=224, help='Размер изображения (H и W)')
    ...
    onnx_path = f'/mnt/c/Users/uservb/Desktop/practice/homework_7/weights/resnet18_{image_size}.onnx'
    pure_trt_path = f'/mnt/c/Users/uservb/Desktop/practice/homework_7/weights/resnet18_{image_size}_pure_trt.engine'
    data_path = f'/mnt/c/Users/uservb/Desktop/practice/homework_5/data/train'

    cuda.init()
    ctx = cuda.Device(0).make_context()

    try:
        dataset = CustomImageDataset(root_dir=data_path, target_size=(image_size, image_size))
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=4)
        ...
        if os.path.exists(onnx_path):
            ...
            pure_trt_time = test_pure_trt_model(
                engine, 
                dataset=dataset, 
                min_batch_size=32, 
                max_batch_size=32, 
        ...
        run_trt_inference(engine, dataloader, input_shape)
